# Infer single image in python environment
...
parser = argparse.ArgumentParser(description = "Muscle_Analysis_Infer_Single_Image")
parser.add_argument('--image_path',  type=str, required=True, default='', 
                    help='Directory of the image to be analyzed')
parser.add_argument('--fasc_model_path', type=str, default='models\model-fasc-WW-unet.h5',
                    help='Directory of the fascicle segmentation model')
parser.add_argument('--apo_model_path', type=str, default='models\model-apo2-nc.h5',
                    help='Directory of the aponeurosis segmentation model')
parser.add_argument('--flip', type=int, default=0, 
                    help='Whether to flip the image horizontally')
parser.add_argument("--mm", type=float, default=10,
                    help="The value of the conversion ratio in millimeters (mm).")
parser.add_argument("--pixel", type=int, default=50,
                    help="The value of the conversion ratio in pixels.")
parser.add_argument('--save_dir',  type=str, default='./output', 
                    help='The directory to save the analyzed image')


opt = parser.parse_args()
image_path = opt.image_path
fasc_model_path = opt.fasc_model_path
apo_model_path = opt.apo_model_path
flip = opt.flip
mm_value = opt.mm
pixel_value = opt.pixel
save_dir = opt.save_dir
# If the save directory doesn't exist, create the directory
...
def infer_image(image, flip, mm_ratio, pixel_ratio, save_dir):

    # IMPORT THE TRAINED MODELS
    # load the aponeurosis model
    # model_apo = load_model('.\\models\\model-apo2-nc.h5', custom_objects={'IoU': IoU})
    model_apo = load_model(apo_model_path, custom_objects={'IoU': IoU})
    # load the fascicle model
    # modelF = load_model('.\\models\\model-fasc-WW-0311a.h5', custom_objects={'IoU': IoU})
    modelF = load_model(fasc_model_path, custom_objects={'IoU': IoU})
...
if __name__=="__main__":
    analysed_image_dir = infer_image(image_path, flip, mm_value, pixel_value, save_dir)
    display_image(analysed_image_dir)
    '''
    python inference_image.py --image_path fasc_images_WW_png\8.png --fasc_model_path models\model-fasc-WW-aunet.h5 --flip 1 --mm 10 --pixel 50 --save_dir ./test_inference_output
    '''